...
$templatecontext['newfrontpage'] = \theme_tilemmetry\toolbox::get_setting('frontpagechooser');
...
if ($templatecontext['newfrontpage']) {
    ...
    $sm = new  \theme_tilemmetry\frontpage\section_manager();
    if (isloggedin()) {
        $usercontext = context_user::instance($USER->id);
        $templatecontext['caneditfrontpage'] = has_capability('theme/tilemmetry:editfrontpage', $usercontext);

        // Migrate previous settings
        if ($templatecontext['caneditfrontpage'] && get_config('theme_tilemmetry', 'createfrontpagesections')) {
            $migrator = new \theme_tilemmetry\frontpage\migrator();
            $hassettings = $migrator->has_settings();
            if (!$hassettings) {
                unset_config('createfrontpagesections', 'theme_tilemmetry');
            } else {
    ...
    $order = get_config('theme_tilemmetry', $orderconfigname);
    $transparentheader = get_config('theme_tilemmetry', 'frontpagetransparentheader');
    if ($transparentheader == 1) {
        $templatecontext['transparentheader'] = $transparentheader;
        $templatecontext['headercolor'] = get_config('theme_tilemmetry', 'frontpageheadercolor');
    }
    ...
    if (get_config('theme_tilemmetry', 'frontpageloader')) {
        $templatecontext['loader'] = \theme_tilemmetry\toolbox::setting_file_url('frontpageloader', 'frontpageloader');
    }

    // Appear animation configuration for js
    $PAGE->requires->data_for_js('appearanimation', get_config('theme_tilemmetry', 'frontpageappearanimation'));
    if (!$style = get_config('theme_tilemmetry', 'frontpageappearanimationstyle')) {
