...
/**
 * SATS Tilemmetry AJAX handler
 * @package    theme_tilemmetry
 * @copyright  (c) 2018 South African Theological Seminary (https://sats.edu.za/)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
 
use theme_tilemmetry\controller\tilemmetry_kernel;
use theme_tilemmetry\controller\tilemmetry_router;
...
$PAGE->set_url('/theme/tilemmetry/request_handler.php', array('action' => $action, 'contextid' => $context->id));

if ($cm !== null) {
    $PAGE->set_cm($cm);
}

$router = new tilemmetry_router();
...
foreach ($contfiles as $contfile) {
    ...
    if (preg_match($pattern, $contfile)) {
        $classname = '\\theme_tilemmetry\\controller\\'.str_ireplace('.php', '', $contfile);
        if (class_exists($classname)) {
            $rc = new ReflectionClass($classname);
            if ($rc->isSubclassOf('\\theme_tilemmetry\\controller\\controller_abstract')) {
...
$kernel = new tilemmetry_kernel($router);
