...
/**
 * General tilemmetry utility functions.
 *
 * Added to a class for the convenience of auto loading.
 *
 * @package   theme_tilemmetry
 * @copyright South African Theological Seminary
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class utility
{
    ...
    public static function get_courses( 
        ...
        foreach ($courses as $k => $course) {
            ...
            $cur_version = $DB->get_record_sql('SELECT * FROM {config_plugins} WHERE plugin = ? AND name = ?', array('theme_tilemmetry', 'version'));
            ...
            if (is_enrolled($context, $USER->id)) {
                ...
                if ($completion->is_enabled()) {
                    ...
                    if (!is_null($percentage)) {
                        ...
                        if ($percentage == 100) {
                            $coursesarray[$count]["coursecompleted"] = get_string('completed', 'theme_tilemmetry');
                        } elseif ($percentage > 0 && $percentage < 100) {
                            $coursesarray[$count]["courseinprogress"] = get_string('resume', 'theme_tilemmetry');
                            ...
                            $coursesarray[$count]["coursetostart"] = get_string('start', 'theme_tilemmetry');
                        }
                    } else {
                        $coursesarray[$count]["coursetostart"] = get_string('start', 'theme_tilemmetry');
                    }
    ...
    public static function get_course_category_filters() {
        ...
        $html .= "<option value='all'>".get_string('allcategories', 'theme_tilemmetry')."</option>";
        ...
        $html .= "<option value='default'>".get_string('sortdefault', 'theme_tilemmetry')."</option>";
        $html .= "<option value='ASC'>".get_string('sortascending', 'theme_tilemmetry')."</option>";
        $html .= "<option value='DESC'>".get_string('sortdescending', 'theme_tilemmetry')."</option>";    
        $html .= "</select></div>";
        ...
        $html .= '<div class="my-10  col-lg-2 col-md-3 hidden-sm-down viewtoggler'.$togglerhidden.'">'.\theme_tilemmetry\utility::get_courses_view_toggler().'</div>';
        return $html;
    }
    ...
    public static function get_courses_by_category($categoryid) {
        ...
        if ($courselist) {
            ...
            if ($others > 0) {
                array_push($labels, get_string('others', 'theme_tilemmetry'));
    ...
    public static function get_recent_user() {
        ...
        foreach ($users as $value) {
            ...
            if ($date == date('d/m/Y')) {
                $date = get_string('today', 'theme_tilemmetry');
            } elseif ($date == date('d/m/Y', time() - (24 * 60 * 60))) {
                $date = get_string('yesterday', 'theme_tilemmetry');
            } else {
    ...
    public static function get_quiz_participation_data($courseid, $limit = 8) {
        ...
        $chartdata = array();
        $index = 0;
        $chartdata['datasets'][0]['label'] = get_string('totalusersattemptedquiz', 'theme_tilemmetry');
        $chartdata['datasets'][1]['label'] = get_string('totalusersnotattemptedquiz', 'theme_tilemmetry');
    ...
    public static function get_slider_data() {
        ...
        if (\theme_tilemmetry\toolbox::get_setting('sliderautoplay') == '1') {
            $sliderdata['slideinterval'] =  \theme_tilemmetry\toolbox::get_setting('slideinterval');
        }

        $numberofslides =  \theme_tilemmetry\toolbox::get_setting('slidercount');

        // Get the content details either static or slider.
        $frontpagecontenttype =  \theme_tilemmetry\toolbox::get_setting('frontpageimagecontent');

        if ($frontpagecontenttype) { // Dynamic image slider.
            $sliderdata['isslider'] = true;
            if ($numberofslides >= 1) {
                for ($count = 1; $count <= $numberofslides; $count++) {
                    $sliderimageurl = \theme_tilemmetry\toolbox::setting_file_url('slideimage'.$count, 'slideimage'.$count);
                    if ($sliderimageurl == "" || $sliderimageurl == null) {
                        $sliderimageurl = \theme_tilemmetry\toolbox::image_url('slide', 'theme');
                    }
                    $sliderimagetext =  format_text(\theme_tilemmetry\toolbox::get_setting('slidertext'.$count));
                    $sliderimagelink =  \theme_tilemmetry\toolbox::get_setting('sliderurl'.$count);
                    $sliderbuttontext =  \theme_tilemmetry\toolbox::get_setting('sliderbuttontext'.$count);
                    if ($count == 1) {
                        $active = true;
                    } else {
                        $active = false;
            ...
            $sliderdata['addtxt'] =  format_text(\theme_tilemmetry\toolbox::get_setting('addtext'));

            $contenttype =  \theme_tilemmetry\toolbox::get_setting('contenttype');
            if (!$contenttype) {
                $sliderdata['isvideo'] = true;
                $sliderdata['video'] =  \theme_tilemmetry\toolbox::get_setting('video');
                $sliderdata['videoalignment'] =  \theme_tilemmetry\toolbox::get_setting('frontpagevideoalignment');
            } elseif ($contenttype) {
                $sliderdata['isimage'] = true;
                $staticimage = \theme_tilemmetry\toolbox::setting_file_url('staticimage', 'staticimage');
                if ($staticimage == "" || $staticimage == null) {
                    $sliderdata['staticimage'] = \theme_tilemmetry\toolbox::image_url('slide', 'theme');
                } else {
    ...
    public static function get_testimonial_data() {
        ...
        $testimonialcount =  \theme_tilemmetry\toolbox::get_setting('testimonialcount');

        if ($testimonialcount >= 1) {
            $testimonial_data['test'] = true;

            for ($count = 1; $count <= $testimonialcount; $count++) {
                $testimonialimageurl = \theme_tilemmetry\toolbox::setting_file_url('testimonialimage'.$count, 'testimonialimage'.$count);

                $testimonialname =  \theme_tilemmetry\toolbox::get_setting('testimonialname'.$count);
                $testimonialdesignation =  \theme_tilemmetry\toolbox::get_setting('testimonialdesignation'.$count);
                $testimonialtext =  \theme_tilemmetry\toolbox::get_setting('testimonialtext'.$count);
                if ($count == 1) {
                    $active = true;
                } else {
                    $active = false;
        ...
        // about us data
        $testimonial_data['aboutus_heading'] = \theme_tilemmetry\toolbox::get_setting('frontpageaboutusheading');
        $testimonial_data['aboutus_desc'] = \theme_tilemmetry\toolbox::get_setting('frontpageaboutustext');
    ...
    public static function get_footer_data($social = false) {
        ...
        for ($i=0; $i < 4; $i++) {
            if ($i == 0) {
                $footer['social'] = array(
                    'facebook' => \theme_tilemmetry\toolbox::get_setting('facebooksetting'),
                    'twitter'  => \theme_tilemmetry\toolbox::get_setting('twittersetting'),
                    'linkedin' => \theme_tilemmetry\toolbox::get_setting('linkedinsetting'),
                    'gplus'    => \theme_tilemmetry\toolbox::get_setting('gplussetting'),
                    'youtube'  => \theme_tilemmetry\toolbox::get_setting('youtubesetting'),
                    'instagram'=> \theme_tilemmetry\toolbox::get_setting('instagramsetting'),
                    'pinterest'=> \theme_tilemmetry\toolbox::get_setting('pinterestsetting'),
                    'quora'=> \theme_tilemmetry\toolbox::get_setting('quorasetting')
                );
                ...
                $title = \theme_tilemmetry\toolbox::get_setting('footercolumn'.$i.'title');
                $content = \theme_tilemmetry\toolbox::get_setting('footercolumn'.$i.'customhtml');
        ...
        // skip footer content if only social
        if (!$social) {
            $footer['bottomtext'] = \theme_tilemmetry\toolbox::get_setting('footerbottomtext');
            $footer['bottomlink'] = \theme_tilemmetry\toolbox::get_setting('footerbottomlink');
    ...
    public static function data_for_messagearea_messages($currentuserid, $otheruserid, $limitfrom = 0, $limitnum = 0, $newest = false, $timefrom = 0) {
        ...
        if (($USER->id != $currentuserid) && !has_capability('moodle/site:readallmessages', $systemcontext)) {
            throw new moodle_exception(get_string('you_do_not_have_permission_to_perform_this_action', 'theme_tilemmetry'));
        }
    ...
    public static function get_user($userorid = false) {
        ...
        if (is_object($userorid)) {
            ...
            throw new coding_exception(get_string('parametermustbeobjectorintegerorstring', 'theme_tilemmetry', $userorid));
        }
    ...
    public static function recent_forum_activity($userorid = false, $limit = 10, $since = null) {
        ...
        $userforums = new \theme_tilemmetry\user_forums($user, $limit);
    ...
    public static function all_ungraded($userid) {
        ...
        $grading = [];
        foreach ($mods as $mod) {
            $class = '\theme_tilemmetry\activity';
    ...
    public static function get_student_progress_view($courseid) {
        ...
        <tr>
        <th class="pl-10">#</th>
        <th class="pl-10"><?php echo(get_string('name', 'theme_tilemmetry'))?></th>
        <th class="pl-10"><?php echo(get_string('status', 'theme_tilemmetry'))?></th>
        <th class="pl-10"><?php echo(get_string('progress', 'theme_tilemmetry'))?></th>
        </tr>
        </thead>
        <tbody>
        <?php
    ...
    public static function get_course_cards_content($wdmdata, $date = 'all') {
        ...
        foreach ($courses as $key => $course) {
            ...
            $coursedata['shortname'] = format_text($course['shortname']);
            $coursedata['courseurl'] = $course['courseurl'];
            $coursedata['coursename']  = format_text($course['coursename']);
            ...
            $coursedata['animation'] = \theme_tilemmetry\toolbox::get_setting('courseanimation');
            if (!\theme_tilemmetry\toolbox::get_setting('enablenewcoursecards')) {
