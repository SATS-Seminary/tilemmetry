...
trait html_form {
    ...
    private function htmlform(&$mform, $formdata, $configdata) {
        ...
        $mform->addElement('checkbox', $title, get_string($title, COMPONENT), get_string($title.'desc', 'theme_tilemmetry'), array('class' => 'ml-0 mr-5 mb-10'));
        ...
        foreach ($configdata['block'] as $kblock => $block) {
            ...
            if ($itemid != "" || $itemid != null) {
                if ($delete) {
                    ...
                    $newitemid = theme_tilemmetry_get_unused_itemid(SEC_HTML);
