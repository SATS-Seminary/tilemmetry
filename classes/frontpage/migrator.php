...
class migrator extends section_manager {
    ...
    /**
     * Get file uploaded in tilemmetry setting
    ...
    private function get_setting_file($filearea, $relativepath) {
        $fullpath = "/{$this->context->id}/theme_tilemmetry/$filearea/0$relativepath";
        $fullpath = rtrim($fullpath, '/');
        return $this->fs->get_file_by_hash(sha1($fullpath));
    }

    /**
     * Create file from tilemmetry settings
    ...
    private function create_file_from_settings($setting, $newarea) {
        $file = get_config('theme_tilemmetry', $setting);
        if ($file != '' && $file = $this->get_setting_file($setting, $file)) {
            $filerecord = [
                'contextid' => $this->context->id,
                'component' => 'theme_tilemmetry',
                'filearea'  => $newarea,
                'itemid'    => theme_tilemmetry_get_unused_itemid($newarea),
                'filepath'  => '/',
    ...
    private function create_slider_from_older_configs() {
        $frontpageimagecontent = get_config('theme_tilemmetry', 'frontpageimagecontent');
        $section = $this->section_configuration('slider');
        $slides = [];
        $defaultslide = $section['configdata']['slide'][0];
        if ($frontpageimagecontent == 0) {
            $contenttype = get_config('theme_tilemmetry', 'contenttype');
            if ($contenttype == 0) {
                return;
            }
            $slides[] = $this->create_slide(
                $defaultslide,
                strip_tags(get_config('theme_tilemmetry', 'addtext')),
                'slideimage' . $i
            );
        } else {
            $slidercount = get_config('theme_tilemmetry', 'slidercount');
            for ($i = 1; $i <= $slidercount; $i++) {
                $slides[] = $this->create_slide(
                    $defaultslide,
                    strip_tags(get_config('theme_tilemmetry', 'slidertext'. $i)),
                    'slideimage' . $i,
                    get_config('theme_tilemmetry', 'sliderbuttontext' . $i),
                    get_config('theme_tilemmetry', 'sliderurl' . $i),
                    $i == 1
                );
            }
        }
    ...
    private function create_aboutus_from_older_configs() {
        ...
        $aboutus = get_config('theme_tilemmetry', 'frontpageblockdisplay');
        if ($aboutus == 1) {
            return;
        }
        $blocks = $section['configdata']['block'];
        $section['configdata']['title']['text'] = get_config('theme_tilemmetry', 'frontpageblockheading');
        $section['configdata']['description']['text'] = strip_tags(get_config('theme_tilemmetry', 'frontpageblockdesc'));
        $section['configdata']['view'] = $aboutus - 1;
        for ($i = 0; $i < 4; $i++) {
            $blocks[$i]['title']['text'] = get_config('theme_tilemmetry', 'frontpageblocksection' . ($i + 1));
            $blocks[$i]['description']['text'] = strip_tags(get_config('theme_tilemmetry', 'frontpageblockdescriptionsection' . ($i + 1)));
            $blocks[$i]['icon'] = 'fa fa-'.get_config('theme_tilemmetry', 'frontpageblockiconsection' . ($i + 1));
            $blocks[$i]['btnlink'] = get_config('theme_tilemmetry', 'sectionbuttonlink' . ($i + 1));
            $blocks[$i]['btnlabel'] = get_config('theme_tilemmetry', 'sectionbuttontext' . ($i + 1));
    ...
    private function create_testimonial_from_older_configs() {
        ...
        $testimonial = get_config('theme_tilemmetry', 'enablefrontpageaboutus');
        if ($testimonial == 0) {
            return;
        }
        ...
        $section['configdata']['title']['text'] = get_config('theme_tilemmetry', 'frontpageaboutusheading');
        $section['configdata']['description']['text'] = strip_tags(get_config('theme_tilemmetry', 'frontpageaboutustext'));
        $section['configdata']['testimonials'] = get_config('theme_tilemmetry', 'testimonialcount');
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $testimonial['name']['text'] = get_config('theme_tilemmetry', 'testimonialname' . ($i + 1));
            $testimonial['designation']['text'] = get_config('theme_tilemmetry', 'testimonialdesignation' . ($i + 1));
            $testimonial['testimonial']['text'] = strip_tags(get_config('theme_tilemmetry', 'testimonialtext' . ($i + 1)));
    ...
    public function create_sections_from_older_configs() {
        ...
        set_config('sections_order', json_encode($order), 'theme_tilemmetry');
