...
class core_renderer extends tilemmetry_renderer
{
    ...
    protected $left;
    protected $tilemmetry = null; // Used for determining if this is a Essential or child of renderer.
    ...
    public function __construct(moodle_page $page, $target)
    {
        ...
        $this->themeconfig = array(\theme_config::load('tilemmetry'));
    }
    ...
    {
        $enableannouncement = \theme_tilemmetry\toolbox::get_setting('enableannouncement');
        $announcement = '';
    ...
    public function standard_head_html()
    {
        ...
        $fontname = ucwords(\theme_tilemmetry\toolbox::get_setting('fontname', 'Roboto'));
        if (empty($fontname)) {
            $fontname = 'Open Sans';
        }
        ...
        $ga_tracking_code = trim(\theme_tilemmetry\toolbox::get_setting('googleanalytics'));
    ...
    {
        $favicon = \theme_tilemmetry\toolbox::setting_file_url('faviconurl', 'faviconurl');
        if (empty($favicon)) {
            return \theme_tilemmetry\toolbox::image_url('favicon', 'theme');
        } else {
            return $favicon;
        }
    }
    ...
    public function search_box_icon_collapsed($id = false)
    {
        ...
        $searchicon = html_writer::tag('button', html_writer::tag('span', get_string('togglesearch', 'theme_tilemmetry'), array('class' => 'sr-only')).$this->pix_icon('a/search', get_string('search', 'search')), array('class' => 'navbar-toggler collapsed mr-0', 'data-target'=>'#site-navbar-search', 'data-toggle' => 'collapse'));
    ...
    protected function render_context_header(\context_header $contextheader)
    {
        ...
        // moved from full_header for proper tilemmetry html structure
        ...
        $overlay = \theme_tilemmetry\toolbox::get_setting('enableheaderbuttons');
        if(!$overlay) {
    ...
    public function should_display_logo()
    {
        ...
        $logoorsitename = \theme_tilemmetry\toolbox::get_setting('logoorsitename');
        $context = array('islogo' => false, 'issitename' => false, 'isiconsitename' => false);
        $checklogo = \theme_tilemmetry\toolbox::setting_file_url('logo', 'logo');
        $checklogomini = \theme_tilemmetry\toolbox::setting_file_url('logomini', 'logomini');

        if (!empty($checklogo)) {
            $logo = $checklogo;
        } else {
            $logo = \theme_tilemmetry\toolbox::image_url('logo', 'theme');
        }

        if (!empty($checklogomini)) {
            $logomini = $checklogomini;
        } else {
            $logomini = \theme_tilemmetry\toolbox::image_url('logomini', 'theme');
        }
        ...
        if ($logoorsitename == 'logo') {
            ...
            $context['siteicon'] = \theme_tilemmetry\toolbox::get_setting('siteicon');
    ...
    public function get_logo_html()
    {
        ...
        $logoorsitename = \theme_tilemmetry\toolbox::get_setting('logoorsitename');
        $siteicon = \theme_tilemmetry\toolbox::get_setting('siteicon');
        $checklogo = \theme_tilemmetry\toolbox::setting_file_url('logo', 'logo');
        $logohtml = '';
        if (!empty($checklogo)) {
            $logo = $checklogo;
    ...
    public function user_menu($user = null, $withlinks = null)
    {
        ...
        $login_dropdown = \theme_tilemmetry\toolbox::get_setting('navlogin_popup');
        ...
        $signinformhtml = '<ul class="dropdown-menu w-350 p-15" role="menu">
                    <form class="mb-0" action="'.get_login_url().'" method="post" id="login">
                        ...
                        <div class="form-group clearfix">
                            ...
                            <a class="float-right" href="'.$forgotpasswordurl.'">'.get_string("forgotpassword", "theme_tilemmetry").'</a>
                        </div>
...
$sitetext = format_string(get_config('theme_tilemmetry', 'brandlogotext'));
    ...
    {
        return \theme_tilemmetry\utility::check_user_admin_cap();
    }
